...
export const OtherProfileBanner = ({ usuario }) => {
  const { usuario: loggedUser } = useUser();
  const [follow, setFollow] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    if (usuario.followersIds.includes(loggedUser.id)) {
      setFollow(true);
    } else {
      setFollow(false);
    }
  }, [usuario.followersIds, loggedUser.id]);
    ...
    if (follow) {
      AxiosConfiguration.delete(
        ...
        .catch((error) => {
          ...
          alert("Hubo un error al dejar de seguir al usuario. Inténtalo de nuevo.");
        })
        .finally(() => {
          setIsLoading(false); 
        });
